...
public partial class DestinationDetailsPage : ContentPage
{
    ...
    private async void OnGalleryImageTapped(object sender, EventArgs e)
    {
        ...
        overlay.Children.Add(closeLabel);
        overlay.Opacity = 0;

        if (Content is Grid mainGrid)
        {
            Grid.SetRowSpan(overlay, Math.Max(mainGrid.RowDefinitions.Count, 1));
            Grid.SetColumnSpan(overlay, Math.Max(mainGrid.ColumnDefinitions.Count, 1));
